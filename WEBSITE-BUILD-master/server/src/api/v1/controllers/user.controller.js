...
const getTripList = asyncHandler(async (req, res) => {
    ...
    console.log("trip list for user: ", userId);
    try {
...
const createWishList = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // find all the wishlists of the user
        const wishList = await Listing.find({ _id: { $in: user.wishList } });
        return res.status(200).json(
            new ApiResponse(200, wishList, "Wish list created successfully")
        );
    } catch (error) {
        throw new ApiError(401,error.message ||  "Unauthorized request")
    }
...
const deleteWishList = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        user.wishList = user.wishList.filter((item) => item._id.toString() !== listId);
        await user.save();
        return res.status(200).json(
            new ApiResponse(200, user.wishList, "Wish list removed/updated successfully")
        );
    } catch (error) {
        throw new ApiError(401,error.message ||  "Unauthorized request")
    }
